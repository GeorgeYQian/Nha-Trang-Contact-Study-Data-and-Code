...
pCont<- ggplot(data=df_contacts_grouped, aes(x=ctagegp_continuous,y=N,fill=factor(contact_time) )) +
  geom_bar(stat="identity")+xlab("Contact Age (years)") + ylab("Number of Contacts")
pCont + scale_fill_brewer(palette = "Accent", name = "Contact Time", labels = c("Short (<5 minutes)", "Medium (5-60 minutes)", "Long (>60 minutes)")) +  theme(legend.position = "none")+ theme_classic() 
