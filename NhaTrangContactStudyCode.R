...
Figure2a <- tidy(pred) %>%
  ...
  geom_point(data = NTrang4thUnder2,
             aes(age, carriageR),
             colour = "darkgreen",
             size = sizeDot) + scale_fill_discrete(breaks = c("trt1", "ctrl", "trt2")) + labs(y = "Carriage Prevalence", x = "Age (years)")
Figure2a
...
carriageIterVectorsAge$agegroups <-
  levels(cut(df_contacts$contact_age, brks, right = F))
...
#Replace column name
names(storePEIValues)[1:ncol(storePEIValues)] <-
  unlist(mapply(function(x, y)
    paste(x, seq(1, y), sep = "_"), varname, n))
...
ProbExposureStore_df <-
  as.data.frame(ProbExposure_df$contact_serial_no)
names(ProbExposureStore_df [1]) <- "ctslno"
gc()

store_PEI_Age %>%
  pivot_longer(!c("contact_serial_no", "ctagegp", "N"),
               names_to = "Posterior",
               values_to = "carriagePrev") %>%
...
#Replace column name
names(ProbExposureStore_df)[2:ncol(ProbExposureStore_df)] <-
  unlist(mapply(function(x, y)
    paste(x, seq(1, y), sep = "_"), varname, n))
...
ProbExposureStoreOnlyNumReshape <-
  melt(ProbExposureStoreOnlyNum)  #the function melt reshapes it from wide to long
...
pPEI1  +
  ...
  theme(strip.background = element_blank(),) + ylab("PEI value") + xlab("Individual A") +
  theme(
    plot.title = element_text(hjust = 0.5),
    #axis.title.x=element_blank(Raccoon),
...
jdat <-
  list(
    "pneu_carr" = df_infants_PEI$infant_pneumo_status,
    "mu" = df_infants_PEI$MeanPEI,
    "sigma" = df_infants_PEI$stdPEI
  )
